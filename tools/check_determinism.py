...
def main() -> None:
    ...
    for path in (run_a, run_b):
        ...
        outp = str(path.resolve())
        # Ensure paths with spaces are properly quoted if not already quoted in the template
        formatted_cmd = args.command
        if '"{input}"' in formatted_cmd:
            formatted_cmd = formatted_cmd.replace('"{input}"', f'"{inp}"')
        elif "'{input}'" in formatted_cmd:
            formatted_cmd = formatted_cmd.replace("'{input}'", f"'{inp}'")
        else:
            formatted_cmd = formatted_cmd.replace("{input}", f'"{inp}"')

        if '"{output}"' in formatted_cmd:
            formatted_cmd = formatted_cmd.replace('"{output}"', f'"{outp}"')
        elif "'{output}'" in formatted_cmd:
            formatted_cmd = formatted_cmd.replace("'{output}'", f"'{outp}'")
        else:
            formatted_cmd = formatted_cmd.replace("{output}", f'"{outp}"')
